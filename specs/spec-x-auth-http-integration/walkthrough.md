...
```
request(opts)
  1. auth.waitUntilSettled()           — unknown 상태 대기
  2. requiresAuth + unauthenticated    — AppError(401) 즉시 throw (fetch 없음)
  3. auth.getToken()                   — token 획득
  4. Authorization: Bearer <token>    — 토큰 있으면 헤더 붙임
  5. attempt()                        — ky 요청
  6. 401 + token 있었으면             — auth.refresh() → attempt() 재시도 1회
     401 + token 없었으면             — refresh 시도 안 함, 즉시 throw
```

### 핵심 결정: token 존재 여부로 refresh 분기

```typescript
if (e.statusCode === 401 && auth && token) {
  // 토큰이 있었던 경우만 refresh — 없었으면 refresh 의미 없음
  await auth.refresh();
  return await attempt();
}
```

`token`이 null이면 (미인증 상태에서 온 401이면) refresh 시도하지 않음.
public endpoint에서 403/401 받아도 불필요한 refresh 방지.
...
요청별 헤더가 Authorization 헤더를 override할 수 있도록 auth headers를 먼저 배치.
...
| `@repo/frontend-http-client` (22 tests) | ✅ PASS |
