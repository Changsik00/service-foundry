# spec-04-02: frontend-sdk — `@repo/frontend-sdk` typed API client (globalThis.fetch + AppError + zod parse)
...
| **Spec ID** | `spec-04-02` |
| **Phase** | `phase-04` |
| **Branch** | `spec-04-02-frontend-sdk` |
| **상태** | Planning |
| **타입** | Feature |
...
`@repo/frontend-sdk` 패키지 신설. **`ky` (fetch 기반 검증된 wrapper)** + **factory pattern** (`createSdk({ baseUrl })`) + **@repo/errors AppError 변환 layer** + **explicit zod schema** 바인딩. ky 의 *retry/timeout/hooks 검증된 기능* 사용 — 알고리즘 직접 박지 않음. `reqId propagation` 은 *frontend 환경 한계* (AsyncLocalStorage 없음) — 호출자가 `headers` 로 명시.
...
```mermaid
flowchart LR
    subgraph fe["frontend-sdk (신규)"]
        Factory["createSdk(options) → Sdk"]
...
1. **`createSdk(options)` factory** export:
   - signature: `createSdk(options: CreateSdkOptions): Sdk`
   - `CreateSdkOptions`: `{ baseUrl, timeoutMs?, retries?, headers? }`
   - 내부 — `ky.create({ prefixUrl, timeout, retry, hooks })` 인스턴스 생성

2. **`Sdk` interface**: backend-http-client 와 *동일 API surface* (학습 비용 0):
...
- [ ] `@repo/frontend-sdk` 신설 (`createSdk` factory + `Sdk` interface)
