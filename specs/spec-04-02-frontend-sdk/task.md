# Task List: spec-04-02 frontend-sdk
...
- [x] Spec ID 확정 및 디렉토리 생성 (`sdd spec new frontend-sdk`)
- [x] spec.md 작성
- [x] plan.md 작성
- [x] task.md 작성 (이 파일)
...
- [ ] `packages/frontend/sdk/{package.json, tsconfig.json, vitest.config.ts}` 박음 (deps: `@repo/errors`, `ky`)
- [ ] `packages/frontend/sdk/src/index.ts` stub (module docstring + export {})
- [ ] `pnpm install` → 25 → 26 workspace projects
- [ ] Commit: `feat(spec-04-02): @repo/frontend-sdk 패키지 scaffold + catalog ky`

본 commit 에 spec-04-02 문서 (spec/plan/task) + backlog auto-update 포함.

---

## Task 3: `createSdk` factory + 단위 테스트 (TDD)

### 3-1. test 작성 (Red)
- [ ] `packages/frontend/sdk/src/index.test.ts` — 9 test (fetch mock):
...
- [ ] stub `createSdk` (throw "not implemented") → typecheck PASS + test Red
- [ ] Commit: `test(spec-04-02): createSdk factory test (Red)`
...
- [ ] 테스트 PASS
- [ ] Commit: `feat(spec-04-02): createSdk factory (ky + AppError 변환 + zod parse)`
...
- [ ] Push: `git push -u origin spec-04-02-frontend-sdk`
- [ ] PR 생성: `gh pr create --base phase-04-frontend-foundation --head spec-04-02-frontend-sdk ...`
- [ ] 사용자 알림
