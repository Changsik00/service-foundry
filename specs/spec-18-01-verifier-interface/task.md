...
### 1-1. 브랜치 생성
- [ ] `git checkout -b spec-18-01-verifier-interface`
- [ ] Commit: 없음 (브랜치 생성만)

### 1-2. 스텁 + 테스트 작성 (TDD Red)
- [ ] `packages/nestjs/auth/src/verifier.ts` 생성:
  - `VerifiedIdentity` 타입
  - `AccessTokenVerifier` 인터페이스
  - `ACCESS_TOKEN_VERIFIER` DI 토큰
  - `NativeVerifier` 스텁 (`throw new Error('not implemented')`)
- [ ] `packages/nestjs/auth/src/verifier.test.ts` 작성:
...
- [ ] `pnpm --filter @repo/nestjs-auth test` → FAIL 확인 (스텁이 throw)
- [ ] Commit: `test(spec-18-01): NativeVerifier 단위 테스트 + 스텁 (Red)`
...
- [ ] `packages/nestjs/auth/src/verifier.ts`의 `NativeVerifier.verify()` 구현:
  - `verifyAccessToken(token, keyStore, { issuer, audience })`
  - `Role.safeParse(result.value.role)`
  - `ACTIVE_ORG_CLAIM` → `orgId` 추출
- [ ] `pnpm --filter @repo/nestjs-auth test` → PASS 확인
- [ ] Commit: `refactor(spec-18-01): NativeVerifier 구현 (Green)`
...
- [ ] `pnpm --filter @apps/api test` → PASS (e2e 회귀 없음)
- [ ] Commit: `refactor(spec-18-01): AuthGuard verifier DI 주입 리팩터 + 모듈 배선`
