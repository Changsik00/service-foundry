# Implementation Plan: spec-04-02 frontend-http-client

## 📋 Branch Strategy

- 신규 브랜치: `spec-04-02-frontend-http-client`
...
> - [x] **client 패턴**: factory (`createHttpClient({ baseUrl })`)
...
```mermaid
flowchart LR
    SDK["@repo/frontend-http-client"] -->|globalThis.fetch| F[Fetch API]
    SDK -->|@repo/errors| AE[AppError]
    SDK -->|peer: zod ^4| Z[(zod 4)]

    Caller["호출자 (Next/Vite app)"] -->|createHttpClient options| SDK
...
| **client 패턴** | factory (`createHttpClient(options)`) | 1회 호출, app 공유. Server/Client Component 모두 자연. NeoAuth 의 *module-level side effect* 정정 |
...
### `@repo/frontend-http-client` (신규)

#### [NEW] `packages/frontend/http-client/package.json`
```json
{
  "name": "@repo/frontend-http-client",
  "version": "0.0.0",
  "private": true,
  "type": "module",
  "exports": {
...
#### [NEW] `packages/frontend/http-client/tsconfig.json`
- `@repo/typescript-config/base` extends
- `lib: ["DOM", "DOM.Iterable", "ES2023"]`

#### [NEW] `packages/frontend/http-client/vitest.config.ts`
- `@repo/vitest-config/node` (jsdom 불필요 — fetch mock 으로 충분)

#### [NEW] `packages/frontend/http-client/src/index.ts`
- 메인 module — 모든 export
...
#### [NEW] `packages/frontend/http-client/src/index.test.ts`
- 단위 테스트 9+ (fetch mock):
  - GET 성공 + schema parse
...
### 수동 검증
1. `grep "export.*HttpClient" packages/frontend/http-client/src/index.ts` — factory + HttpClient export
