...
## 8. 토큰 승격 규칙 (fill-forward)

> 토큰 세트는 **미리 완성하지 않는다** — 구현(spec-x-ui-tokens, auth-screens, 이후 모든 화면)에서
> 필요가 증명될 때 채워 간다. shadcn 기본값이 DESIGN.md 와 다른 곳, 매핑이 비어 있는 곳을
> 만나면 아래 절차를 따른다.
...
필요한 값이 토큰에 없다
  │
  ├─ 1. 파생 가능한가? → 기존 시스템에서 파생 (새 토큰 아님)
  │      회색   → ink opacity 단계 추가 (rgb(var(--ink) / N))
  │      간격   → 4px 그리드의 새 배수
  │      그림자 → ring + 기존 elevation 보간
  │
  ├─ 2. 1회성인가? → 인라인 임의값 허용 (w-[N px] 등) — 토큰 만들지 않음
  │
  └─ 3. 2회째 등장 → 토큰 승격 (필수):
         ① @theme inline / :root 에 추가 (semantic 이름으로)
         ② 본 문서(TOKEN.md) 매핑 표에 한 줄 추가
         ③ 값의 성격이 디자인 결정이면 DESIGN.md 해당 섹션에도 반영
         ②③ 없이 ①만 하면 안 됨 — 코드·문서 동기화가 계약
```
...
| 영역 | 규칙 |
|---|---|
| **새 hue (색상)** | 파생 불가능한 새 색은 **사용자 승인 필수** — 미학 정체성은 사용자 결정 영역 |
| radius 새 단계 | 금지 — 6/8/12/16/999 5단계가 전부 (위계가 의도, DESIGN §4.2) |
| 회색·간격·그림자·폰트 크기 | 파생 규칙 안에서 에이전트 재량 |

### 왜 이 방식인가

- 선제 완성 = filler: 쓰이지 않는 토큰은 유지 비용만 남기고, 실수요와 어긋난 채 굳는다
- 코드가 생기면 코드가 문서보다 강한 기준 — 문서는 매핑·결정만 기록하고 나머지는 구현이 채운다
- "재사용 가능"의 조작적 정의 = 2회 사용 시 승격. 그 전까지는 재사용이 아니라 추측이다
