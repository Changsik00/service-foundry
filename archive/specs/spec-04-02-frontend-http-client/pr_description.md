# feat(spec-04-02): `@repo/frontend-http-client` — ky 기반 typed API client + AppError 변환

> Phase 4 (Frontend Foundation) 두 번째 spec. `@repo/frontend-http-client` 신설 — **ky** 기반 cross-env typed API client + `AppError` 변환 layer + explicit zod schema binding. NeoAuth `auth-http` 코드 리뷰 후 *형편없는 점 7개 모두 정정*.
...
- [x] **`@repo/frontend-http-client` 신설** (`packages/frontend/http-client/`):
  - `createHttpClient(options)` factory — `ky.create({ baseUrl, timeout, retry, hooks })`
...
- ✅ `pnpm test`: **175 test PASS** (frontend-http-client 9 신규 + 기존 166)
- ✅ `depcruise`: **0 violations** (107 modules / 183 dependencies)

### test 분포 (9 신규 — `@repo/frontend-http-client`)

| describe | test 수 | 검증 |
|---|:---:|---|
| `createHttpClient` | 9 | GET 200+schema / 404 BAD_REQUEST / 500 retry UPSTREAM / timeout TIMEOUT / network NETWORK / validation VALIDATION / POST no-retry / POST retry / headers+body |

### 수동 검증

```bash
# 1. 신규 패키지
ls packages/frontend/http-client/src/
...
# 3. exports
grep "^export" packages/frontend/http-client/src/index.ts
# → HttpMethod / CreateHttpClientOptions / HttpRequestOptions / HttpClient / createHttpClient
```
...
- **walkthrough**: `specs/spec-04-02-frontend-http-client/walkthrough.md` (결정 13 + 협의 7 + 진행 5 + 검증 + 발견 8 + 이월 6)
...
- [x] `@repo/frontend-http-client` 신설 (`createHttpClient` factory + `HttpClient` interface)
