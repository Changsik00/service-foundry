# Walkthrough: spec-04-02 frontend-http-client

> phase-04 두 번째 spec. `@repo/frontend-http-client` 신설 — **ky** 기반 typed API client + AppError 변환 layer + explicit zod schema. NeoAuth `auth-http` 코드 리뷰 후 *형편없는 점 정정* (타입 안전성 / Error subclass / factory pattern / retry 옵션 노출).
...
| client 패턴 | factory / class / hooks | **factory** (`createHttpClient`) | NeoAuth 의 *module-level side effect* (`new HttpClient(env)`) 정정. Server/Client Component 모두 자연 |
...
- `packages/frontend/http-client/{package.json, tsconfig.json, vitest.config.ts, src/index.ts}` 박음
  - deps: `@repo/errors` + `ky`
  - peer: `zod ^4.0.0`
...
- stub `createHttpClient` (throw "not implemented") + `CreateHttpClientOptions/HttpRequestOptions/HttpClient` 시그니처
- typecheck PASS + test 9/9 Red

**Green (`b6ef58e`)**:
- `createHttpClient` 본체 — `ky.create({ baseUrl, timeout, retry, hooks })`
...
| 패키지 | test 수 | 상태 |
|---|:---:|:---:|
| `@repo/frontend-http-client` (신규) | 9 | ✓ |
| 기타 16 패키지 (변경 없음) | 166 | ✓ |
| **합계** | **175** | **all green** |
...
이전 (PR #24 직후) 103 modules / 175 deps → +4 module / +8 dep (frontend-http-client + ky transitive).
...
| 6. module-level side effect | ✅ factory pattern (`createHttpClient(options)`) — test 주입 가능 |
...
| **test 수** | 9 신규 (`@repo/frontend-http-client`) — 전체 175 PASS |
| **depcruise** | 0 violations (107 modules / 183 deps, +4 / +8) |
| **신규 패키지** | `@repo/frontend-http-client` |
| **신규 catalog** | `ky ^2.0.2` |
