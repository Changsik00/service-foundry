...
- [ ] apps/admin 별도 앱 여부 결정 (phase-09) — ~~vs web-vite route~~ web-vite 는 ADR-0025 로 삭제됨 (2026-06-10)
...
- [x] ~~**web-next CSRF 403 자가복구**~~ **부분 해소**: `apps/web-next/src/lib/auth-api.ts` `withCsrfRetry` 구현 완료 (2026-06-09). **잔여**: 401 자동 갱신 → `spec-x-auth-token-refresh-interceptor` (spec-x 대기), password-reset/email-verify 클라이언트 흐름 (~~web-vite CSRF 헤더~~ ADR-0025 로 소멸)
