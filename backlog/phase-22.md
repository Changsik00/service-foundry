# phase-22: Deploy (k8s manifest 예제)

> 보일러플레이트의 마지막 단계 — 배포 예제(컨테이너 오케스트레이션).
> ⚠️ 2026-06-02 재배치: CI/CD(PR 게이트·changesets·docker publish)는 **phase-14 에서 완료**, knip/depcruise 게이트는 phase-15 에서 완료. 본 phase 는 **k8s manifest 예제 잔여분**만. (멀티테넌트 SaaS 로드맵 확정으로 deploy 를 맨 뒤 phase-22 로 이동 — infra 전에 멀티테넌시·계정·인가·데이터·어드민·빌링 선행, → ADR-0022.)
...
- **Given**: spec-22-01 머지됨, 로컬 클러스터 기동.
