...
export const createSdk = (options: CreateSdkOptions): Sdk => {
  ...
  const request = async <T>(opts: SdkRequestOptions<T>): Promise<T> => {
    ...
    // ky 의 prefixUrl + path 는 / 로 시작하면 안 됨 — strip
  ...
  return {
    request,
    get: <T>(path: string, opts?: Partial<SdkRequestOptions<T>>) =>
      request<T>({ ...opts, method: "GET", path }),
    post: <T>(path: string, body?: unknown, opts?: Partial<SdkRequestOptions<T>>) =>
      request<T>({ ...opts, method: "POST", path, body }),
    put: <T>(path: string, body?: unknown, opts?: Partial<SdkRequestOptions<T>>) =>
      request<T>({ ...opts, method: "PUT", path, body }),
    delete: <T>(path: string, opts?: Partial<SdkRequestOptions<T>>) =>
      request<T>({ ...opts, method: "DELETE", path }),
    patch: <T>(path: string, body?: unknown, opts?: Partial<SdkRequestOptions<T>>) =>
