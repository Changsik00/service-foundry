...
- **워크스페이스별 `ignoreDependencies` (scaffolding)**: 선언만 되고 아직 미배선인 의도적 deps. 예) `apps/api` 의 `undici`. YAGNI-면제 철학상 삭제하지 않고 보존. (`@repo/backend-auth-rate-limit` 는 phase-15~16 에서 배선 완료 → ignore 제거됨.)
